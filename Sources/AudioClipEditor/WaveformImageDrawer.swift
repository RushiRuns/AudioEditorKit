    ...
    func render(
        ...
        let task = Task(priority: qos.taskPriority) {
            try await withTaskCancellationHandler {
                try await waveformAnalyzer.samples(fromAudioAt: audioAssetURL, count: sampleCount, qos: qos)
            } onCancel: {
...
private extension WaveformImageDrawer {
    private func stripeCount(_ configuration: Waveform.Configuration) -> Int {
        if case .striped = configuration.style {
            Int(configuration.size.width * configuration.scale) / stripeBucket(configuration)
        } else {
            0
        }
    }
