...
public struct LinearWaveformRenderer: WaveformRenderer {
    ...
    public func path(samples: [Float], with configuration: Waveform.Configuration, lastOffset: Int = 0, position: Waveform.Position = .middle) -> CGPath {
    ...
    public func render(samples: [Float], on context: CGContext, with configuration: Waveform.Configuration, lastOffset: Int = 0, position: Waveform.Position = .middle) {
    ...
    private func stripeBucket(_ configuration: Waveform.Configuration) -> Int {
        if case let .striped(stripeConfig) = configuration.style {
            Int(stripeConfig.width + stripeConfig.spacing) * Int(configuration.scale)
        } else {
            0
        }
    }
    ...
    private func draw(samples: [Float], path: CGMutablePath, with configuration: Waveform.Configuration, lastOffset: Int, sides: Sides, position: Waveform.Position = .middle) -> CGMutablePath {
        ...
        for (index, sample) in samples.enumerated() {
            let adjustedIndex: Int = switch sides {
            case .up, .both: index
            case .down: samples.count - index
            }
